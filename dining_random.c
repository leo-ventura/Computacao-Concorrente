...
// SOLUTION:
// When picking the first chopstick, the philosophers will
// randomly choose between left and right. To avoid deadlocks,
// they will put it down if the other chopstick is being used.
// This solution is an implementation of the third suggested
// solution in http://wiki.c2.com/?DiningPhilosophers
...
int main()
{
    ...
    int phil_num[N];
    int i;
    // each thread will simulate the behavior of one philosopher
...
void take_fork(int ph_num)
{
	...
	int first = (ph_num + random_choice) % N;
	...
	int second = (ph_num + (1 - random_choice)) % N;
        printf("Philosopher %d now try taking chopstick %d\n", ph_num + 1, second + 1);
        if (sem_trywait(&chopsticks[second]) == 0)
        {
            // got second chopstick
            printf("Philosopher %d took chopstick %d and %d, and now is Eating\n",
                    ph_num + 1, first + 1, second + 1);
            sleep(1);
            break;
        }
        else
        {
            // could not get second chopstick, releasing the first chopstick
            printf(ANSI_COLOR_RED
                   "Philosopher %d unable to get both chopsticks %d and %d, releasing first one\n"
                   ANSI_COLOR_RESET, ph_num + 1, first + 1, second + 1);
